...
describe('TaskService', () => {
    ...
    describe('createTask', () => {
        ...
        it('Should return ValidationError when title it not passed in', async () => {
            await expect(TaskService.createTask({} as ICreateTaskRequest))
                .rejects
                .toThrow(ValidationError)
            await expect(TaskService.createTask({} as ICreateTaskRequest))
                .rejects
    ...
    describe('updateTask', () => {
        it('Should update existing task', async () => {
            ...
            (TaskModel.findByIdAndUpdate as jest.Mock).mockResolvedValue(mockUpdatedTask);
        ...
        it('Should throw a ValidationError when id is not given in the function', async () => {
            await expect(TaskService.updateTask("", {} as IUpdateTaskRequest))
                .rejects
                .toThrow(ValidationError);
            await expect(TaskService.updateTask("", {} as IUpdateTaskRequest))
                .rejects
        ...
        it('Should throw a ValidationError when ID is invalid', async () => {
            await expect(TaskService.updateTask("123", {} as IUpdateTaskRequest))
                .rejects
                .toThrow(ValidationError);
            await expect(TaskService.updateTask("123", {} as IUpdateTaskRequest))
                .rejects
